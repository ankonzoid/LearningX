...
class Agent:
    ...
    def get_action(self, state, state_global, brain, env):
        # Explore actions
        def explore_actions_allowed(state, state_global, env):
            actions_explore_allowed = env.allowed_actions(state, state_global)
            return actions_explore_allowed
        # Choose highest value action
        def argmax_Q_actions_allowed(Q, state, state_global, env):
            actions_allowed = env.allowed_actions(state, state_global)
        ...
        if random.uniform(0, 1) < self.epsilon:
            actions_explore_allowed = explore_actions_allowed(state, state_global, env)
            return np.random.choice(actions_explore_allowed)
        else:
            actions_Qmax_allowed = argmax_Q_actions_allowed(brain.Q, state, state_global, env)
