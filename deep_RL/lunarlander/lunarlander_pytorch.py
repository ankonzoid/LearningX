...
class Agent():
    ...
    def learn(self, experiences, gamma):
        ...
        Q_targets_next = self.qnetwork_target(next_states).detach().max(1)[0].unsqueeze(1) # get Qmax for next states
        Q_targets = rewards + (gamma * Q_targets_next * (1 - dones)) # compute Q targets for current states
        Q_expected = self.qnetwork_local(states).gather(1, actions) # inference expected Q-values
        loss = F.mse_loss(Q_expected, Q_targets) # compute loss
        self.optimizer.zero_grad() # optimize by minimizing loss
        loss.backward()
        self.optimizer.step()
...
for i_episode in range(1, n_episodes+1):
    ...
    if i_episode % 100 == 0:
        ...
        plt.ylabel('Score')
        plt.xlabel('Episode #')
        plt.savefig("output/rew_{}.png".format(env.unwrapped.spec.id))
        plt.close()
